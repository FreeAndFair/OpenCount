...
def dt1(f):
    ...
    for q in range(1, n - 1):
        s1 = ((f[q] + pow(q, 2)) - (f[v[k]] + pow(v[k], 2))) / \
            (2 * q - 2 * v[k])
        while s1 <= z[k]:
            k -= 1
            s1 = ((f[q] + pow(q, 2)) - (f[v[k]] + pow(v[k], 2))) / \
...
def process_one(args):
    try:
        ...
        #                                         str(do_flip)),
        #                I1)
        rejected_hash = rejected_hashes.get(
            imP, None) if rejected_hashes else None
        accepted_hash = accepted_hashes.get(
            imP, None) if accepted_hashes else None
        # perform matching for all digits
        # return best matching digit
        # mask out
        # res := (str ocr_str, list patches, list bbs, list scores, list
        # matched_keys)
...
def digitParse(digit_hash, imList, bbSearch, nDigits, flipmap=None, hspace=20,
    ...
    if nProc < 2:
        results = []
        for x in imList:
            results.append(process_one((x, digit_hash, imList, bbSearch, nDigits,
                                        hspace, rejected_hashes, accepted_hashes, flipmap, queue_progress)))
