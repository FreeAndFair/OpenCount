...
def gridAlign(I, Iref, r, fac1=1.5, fac2=.5, vCells=3, hCells=5, verbose=False):
    ...
    # part 1
    fac = 1.5
    (rOut, rOff) = expand(r[0], r[1], r[2], r[
        3], I.shape[0] - 1, I.shape[1] - 1, fac)
    ...
    # part 2
    fac = .1
    (rOut, rOff) = expand(rOff[0], rOff[1], rOff[2],
                          rOff[3], I1.shape[0] - 1, I1.shape[1] - 1, fac)
    Iref2 = Iref1[rOut[0]:rOut[1], rOut[2]:rOut[3]]
    I2 = I1T[rOut[0]:rOut[1], rOut[2]:rOut[3]]

    I2T = imagesAlign(I2, Iref2, trfm_type='projective',
                      vCells=vCells, hCells=hCells)[1]
    ...
    if verbose:
        ...
        figure(0)
        imshow(Ipt0 - Irefpt)
        title('Initial patch diff')
        colorbar()

        figure(1)
        imshow(I1T - Iref1)
        title('After first round align')
        colorbar()

        figure(2)
        imshow(Dabs)
        title('Final patch diff')
        colorbar()
