...
def padWithBorderHandling(M, i1exp, i2exp, j1exp, j2exp):
    ...
    Mpad[i1exp:i1exp + leftedge.shape[0],
         0:j1exp] = np.tile(leftedge, (j1exp, 1)).T
    ...
    Mpad[0:i1exp, j1exp:j1exp + topedge.shape[0]
         ] = np.tile(topedge, (i1exp, 1))
...
def lkSmallLarge(patch, I, i1, i2, j1, j2, pixPad=5, minArea=np.power(2, 17)):
    ...
    IO = lk.imagesAlign(Ic, patchPad, trfm_type='rigid',
                        fillval=Ibg, minArea=minArea)
    Ireg = IO[1]
...
def find_patch_matchesV1(I, bb, imList, threshold=.8, rszFac=.75, bbSearch=None,
    ...
    bb[2] = bb[2] * rszFac
    bb[3] = bb[3] * rszFac
    [bbOut, bbOff] = expand(bb[0], bb[1], bb[2], bb[3],
                            I.shape[0], I.shape[1], padPatch)
    ...
    for cur_i, imP in enumerate(imList):
        ...
        outCv = cv.CreateMat(
            I1.shape[0] - patch.shape[0] + 1, I1.shape[1] - patch.shape[1] + 1, cv.CV_32F)
...
def digitParse(digit_hash, imList, bbSearch, nDigits, do_flip=False):
    ...
    for imP in imList:
        ...
        result_meta = sorted(
            result_meta, key=lambda result_meta: result_meta[2])
