...
def imagesAlign1(I, Iref, H0=np.eye(3, dtype=np.float32),
    ...
    if np.prod(wh) < minArea:
        ...
        (H, errx) = imagesAlign1(I1, Iref1, H0=H0,
...
def ds2H(ds, wts):
    ...
    # # 6. rotation z
    ct = math.cos(wts[5] * ds[5])
    st = math.sin(wts[5] * ds[5])
    Hs[5, :2, :2] = np.array([[ct, st], [-st, ct]])
    # # 7. rotation x
    ct = math.cos(wts[6] * ds[6])
    st = math.sin(wts[6] * ds[6])
    Hs[6, 1, 1] = ct
    Hs[6, 1, 2] = -st
    Hs[6, 2, 1] = st
    Hs[6, 2, 2] = ct
    # # 8. rotation y
    ct = math.cos(wts[7] * ds[7])
    st = math.sin(wts[7] * ds[7])
    Hs[7, 0, 0] = ct
    Hs[7, 0, 2] = -st
    Hs[7, 2, 0] = st
    Hs[7, 2, 2] = ct
