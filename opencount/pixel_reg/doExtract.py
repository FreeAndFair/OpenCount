...
def extractTargets(I, Iref, bbs, verbose=False):
    ...
    H1, I1, err = imagesAlign(
        I, IrefM, fillval=1, trfm_type='translation', rszFac=rszFac)
    if(verbose):
    ...
    for i in range(bbs.shape[0]):
        ...
        Hc1, Ic1, err = imagesAlign(
            Ic, Irefc, fillval=1, rszFac=rszFac, trfm_type='rigid')

        targ = np.copy(sh.cropBb(Ic1, bbOff))

        # unwind the transformations to get the global location of the target
        rOut_tr = pttransform(I, np.linalg.inv(
            H1), np.array([bbOut[2], bbOut[0], 1]))
        rOff_tr = pttransform(Ic, np.linalg.inv(
            Hc1), np.array([bbOff[2], bbOff[0], 1]))
        ...
        # weird bug in imsave where if the matrix is all ones, it saves as pure
        # black
        targ[0, 0] = .99
...
def extractTargetsRegions(I, Iref, bbs, vCells=4, hCells=4, verbose=False, balP=None,
    ...
    # ratio={2:.5f}'.format(err_galign, orig_err, err_rel)
    if FLAG_UNDO_GALIGN:
        print "(Info) Undoing galign (err_rel={0}) [wrote: undo_galign.log]".format(err_rel)
        f = open('undo_galign.log', 'a')
        print >>f, "balP={0}, orig_err={1}, err_galign={2}, err_rel={3}".format(
            balP, orig_err, err_galign, err_rel)
        f.close()
    ...
    if method_lalign == LALIGN_NONE:
        ...
        for i, bb in enumerate(bbs):
            # bb := [i1, i2, j1, j2, targetID]
            bbExp, bbOff = sh.expandBbsSingle(
                bb, I1.shape[0], I1.shape[1], pFac)
            ...
            rOut_tr = pttransform(I, np.linalg.inv(
                H1), np.array([bbExp[2], bbExp[0], 1]))
            rOff_tr = pttransform(I_patch, np.linalg.inv(
                H2), np.array([bbOff[2], bbOff[0], 1]))
            targLocGl = np.zeros(4)
            ...
            # weird bug in imsave where if the matrix is all ones, it saves as
            # pure black
        ...
        for i in range(vCells):
            for j in range(hCells):
                ...
                (bbOut, bbsOff) = sh.expandBbs(
                    bbs1, I.shape[0], I.shape[1], pFac)
                ...
                for k in range(bbsOff.shape[0]):
                    ...
                    # unwind the transformations to get the global location of
                    # the target
                    rOut_tr = pttransform(
                        I, H1_inv, np.array([bbOut[2], bbOut[0], 1]))
                    rOff_tr = pttransform(
                        Ic, Hc1_inv, np.array([bbOff1[2], bbOff1[0], 1]))
                    ...
                    # weird bug in imsave where if the matrix is all ones, it
                    # saves as pure black
...
def writeMAP(imgs, targetDir, targetDiffDir, imageMetaDir, balId,
    ...
    relpath_root = os.path.relpath(os.path.abspath(
        os.path.split(balP)[0]), os.path.abspath(voted_rootdir))
    ...
    imgmetaout_rootdir = os.path.normpath(os.path.join(
        imageMetaDir, relpath_root, imgname, "page{0}".format(page)))

    # try: os.makedirs(targetmetaout_rootdir)
    # except: pass
    try:
        os.makedirs(targetdiffout_rootdir)
    except:
        pass
    try:
        os.makedirs(imgmetaout_rootdir)
    except:
        pass

    curid = str(id(mp.current_process()))
    radix_sort_dir = os.path.join(os.path.abspath(
        projdir), "extracted_radix/" + curid)
    # print radix_sort_dir
    try:
        os.makedirs(radix_sort_dir)
    except:
        pass
    ...
    for uid, img, bbox, Idiff in imgs:
        ...
        if not HACK_SANTACRUZ:
            ...
            avg_intensity = np.sum(
                _INTEREST) / float(_INTEREST.shape[0] * _INTEREST.shape[1])
        ...
        data = array.array("B", np.floor(
            img.flatten()).astype('uint8')).tostring()
    ...
    imgmeta_outpath = os.path.join(
        imgmetaout_rootdir, "{0}_meta.p".format(imgname))
...
def quarantineCheckMAP(jobs, targetDiffDir, quarantined_outP, img2bal, bal2targets, imageMetaDir=[]):
    ...
    for K in range(len(jobs)):
        # job := [tplImgs, tplPaths, tplFlips, bbsL_i, balL_i, balid,
        # balL_flips, targetDiri, targetDiffDir_i, imageMetaDir_i, voteddir,
        # projdir, queue, result_queue]
        job = jobs[K]
        balL = job[4]
...
def convertImagesWorkerMAP(job):
    ...
    try:
        (tplImgs, tplPaths, tpl_flips, bbsL, balL, balId, balL_flips, targetDir, targetDiffDir,
         imageMetaDir, voted_rootdir, projdir, queue, result_queue, method_galign, method_lalign) = job
        t0 = time.clock()
        ...
        for i, imP in enumerate(balL):
            if method_galign == GALIGN_NORMAL:
                balImg = sh.standardImread_v2(
                    imP, flatten=True, dtype='float32', normalize=True)
            else:
                balImg = sh.standardImread_v2(
                    imP, flatten=True, dtype='uint8', normalize=False)
            if balL_flips[i]:
            ...
            if tplImg == None:
                ...
                if method_galign == GALIGN_NORMAL:
                    tplImg = sh.standardImread_v2(
                        tplPaths[side], flatten=True, dtype='float32', normalize=True)
                else:
                    tplImg = sh.standardImread_v2(
                        tplPaths[side], flatten=True, dtype='uint8', normalize=False)
                if tpl_flips[side]:
...
def extract_targets(group_to_ballots, b2imgs, img2b, img2page, img2flip, target_locs_map,
    ...
    # targetMetaDir, imgMetaDir, queue], ...]
    jobs = []
    # 1.) Create jobs
    manager = mp.Manager()
    queue = manager.Queue()
    ...
    _lalignnames = {LALIGN_NORMAL: "NORMAL",
                    LALIGN_CV: "CV", LALIGN_NONE: "NONE"}
    print "(Info) METHOD_GALIGN={0} METHOD_LALIGN={1}".format(_galignnames[method_galign], _lalignnames[method_lalign])
    # In Bytes, e.g. 950 MB. Limit memory usage of template pre-loading
    MAX_SIZE = 950 * (1e6)
    cur_size = 0
    if os.path.exists(os.path.join(projdir, 'group_to_Iref.p')):
        group_to_Iref = pickle.load(
            open(os.path.join(projdir, 'group_to_Iref.p'), 'rb'))
    else:
        group_to_Iref = None
    ...
    if os.path.exists(os.path.join(projdir, 'quarantinedgroups_seltargets.p')):
        ...
        groups_quar = pickle.load(
            open(os.path.join(projdir, 'quarantinedgroups_seltargets.p')))
    else:
        groups_quar = set()
    ...
    for i, (groupID, ballotIDs) in enumerate(sorted(group_to_ballots.iteritems(), key=lambda t: -len(t[1]))):
        ...
        for i, blankpath in enumerate(blankpaths_ordered):
            ...
            if cur_size >= MAX_SIZE:
                ...
                if method_galign == GALIGN_NORMAL:
                    Iblank = sh.standardImread_v2(
                        blankpath, flatten=True, dtype='float32', normalize=True)
                else:
                    Iblank = sh.standardImread_v2(
                        blankpath, flatten=True, dtype='uint8', normalize=False)
                if isflip:
    ...
    avg_intensities, bal2targets = convertImagesMasterMAP(targetDir, targetMetaDir, imageMetaDir, jobs, img2b,
