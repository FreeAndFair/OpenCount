...
def evalPatchSimilarity2(I, patch, debug=False):
    ...
    IO = imagesAlign(I1c, patchPad, trfm_type='rigid',
                     hCells=hCells, vCells=vCells, minArea=np.power(2, 15))
    if debug:
        pdb.set_trace()
    Ireg = IO[1]
...
def dist2patches(patchTuples, scale, debug=False):
    ...
    for idx in range(len(patchTuples)):
        ...
        attrval = pt[2]
        flag = False
        # A fix for a very bizarre openCv bug follows..... [check
...
def createPatchTuples(I, attr2pat, R, flip=False):
    ...
    (rOut, rOff) = sh.expand(R[0], R[1], R[
        2], R[3], I.shape[0], I.shape[1], pFac)
...
def groupImagesWorkerMAP(job):
    try:
        ...
        (ballotid, imgpaths, attrtype, bb, attr2pat, isflip,
         scale, patchDestDir, queue, queue_progress) = job
        ...
        # TODO: arbitrary choice, is this ok?
        exmpl_imP, firstPat = attr2pat.values()[0][0]
        rszFac = sh.resizeOrNot(firstPat.shape, sh.MAX_PRECINCT_PATCH_DIM)
        sweep = np.linspace(scale, rszFac, num=np.ceil(
            np.log2(len(attr2pat))) + 2)
        ...
        for sc in sweep:
            (scores, locs, exemplar_idxs) = dist2patches(
                patchTuples, sc, debug=debug)
            sidx = np.argsort(scores)
            # reverse for descend
            sidx = sidx[::-1]
            mid = np.ceil(len(sidx) / 2.0)
        ...
        I1c = I1[bestLocG[0]:bestLocG[0] + P1.shape[0],
                 bestLocG[1]:bestLocG[1] + P1.shape[1]]
        ...
        IO = imagesAlign(I1c, P1, trfm_type='rigid',
                         rszFac=rszFac, minArea=np.power(2, 15))
        Ireg = np.nan_to_num(IO[1])
        # RESULT: [[int ballotid, attrtype, dict outdict], ...]

        result = doWriteMAP(finalOrder, Ireg, IO[
                            2], attrtype, patchDestDir, ballotid, bestExemplarIdx)
        queue.put(result)
...
def estimateScale(attr2pat, img2flip, superRegion, rszFac, stopped):
    ...
    pool = mp.Pool(processes=nProc,
                   initializer=templateSSWorker_init, initargs=[queue])
...
def groupByAttr(bal2imgs, img2page, img2flip, attrName, side, attrMap,
    ...
    """
    if deleteall:
        if os.path.exists(patchDestDir):
            shutil.rmtree(patchDestDir)
    ...
    #     attr2pat.
    # multexemplars_map: maps {attrtype: {attrval: ((str patchpath_i, str
    # blankpath_i, (x1,y1,x2,y2)), ...)}}
    ...
    if nProc < 2:
        ...
        pool.map_async(groupImagesWorkerMAP, jobs,
    ...
    # TODO: quarantine on grouping errors. For now, just let alignment check
    # handle it
...
-- not running grouping for this.".format(attrtype)
            ...
            # Run grouping on /every/ voted ballot, minus quarantined/discarded
            # ballots
