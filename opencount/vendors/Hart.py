...
class HartVendor(Vendor):
    ...
    def decode_ballots(self,
        ...
        # ...]}
        decoded_results_map = decode_ballots(
            ballots, topbot_paths, manager, queue, skipVerify=skipVerify, cache=cache)
        ...
        total_bytes = asize.asizeof(
            decoded_results_map) + asize.asizeof(flipmap) + asize.asizeof(bbstripes_map)
        ...
        if verified_results:
            ...
            for bc_val, tups in verified_results.iteritems():
                for (imgpath, (x1, y1, x2, y2), bbstripe_i) in tups:
                    img_bc_temp.setdefault(imgpath, []).append(
                        (bbstripe_i, bc_val))
...
def _do_decode_ballots(ballots, (topbot_paths, skipVerify, cache), queue=None):
    ...
    # maps {int ballotid: [(bcs_side0, isflipped_side0, bbs_side0,
    # bbstripes_side0), ...]}
    results = {}
    for ballotid, imgpaths in ballots.iteritems():
        ...
        for imgpath in imgpaths:
            if cache and imgpath in cache:
                ...
                continue
            try:
                bcs, isflipped, bbs, bbstripes_dict = hart.decode(
                    imgpath, topbot_pairs, skipVerify=skipVerify)
...
def main():
    ...
    decoded_results = decode_ballots(
        bal2imgs_in, topbot_paths, manager, queue, skipVerify=True, N=args.numprocs)
