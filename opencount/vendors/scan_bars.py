...
def parse_patch(patch, marksize, gap=7, LEN=None,
    ...
    flat = cv.CreateMat(
        1, w, cv.CV_32S) if orient == HORIZONTAL else cv.CreateMat(h, 1, cv.CV_32S)
    dim = 0 if orient == HORIZONTAL else 1

    cv.Reduce(patch, flat, dim=dim, op=cv.CV_REDUCE_SUM)
    flat_np = np.asarray(
        flat)[0] if orient == HORIZONTAL else np.asarray(flat)[:, 0]
...
def scan_line(data, w_mark, pix_on, pix_off, gap, LEN=None,
    ...
    def parse_mark_maybe(data, idx, check_on):
        ...
        # print "== idx={0} data[idx]={4} run_len={1} check_on={2}
        # is_mark={3}".format(idx, run_len, check_on, is_mark, data[idx])
        ...
        for idx, (sym, x1) in enumerate(syms):
            if orient == HORIZONTAL:
                cv.Line(Icolor, (x1, 0), (x1, h_mark - 1),
                        cv.CV_RGB(230, 0, 0))
                cv.Line(Icolor, (x1 + w_mark, 0),
                        (x1 + w_mark, h_mark - 1), cv.CV_RGB(0, 0, 230))
            else:
                y1 = x1
                cv.Line(Icolor, (0, y1), (w_mark - 1, y1),
                        cv.CV_RGB(230, 0, 0))
                cv.Line(Icolor, (0, y1 + h_mark - 1), (w_mark -
                                                       1, y1 + h_mark - 1), cv.CV_RGB(0, 0, 230))
