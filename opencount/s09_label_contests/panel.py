...
class LabelContest(wx.Panel):
    ...
    def gatherData(self):
        ...
        # len(groupedtargets[a][b]) is the number of targets in contest B of
        # template A
        ...
        for groupID, contests_sides in target_locs_map.iteritems():
            ...
            if groupID in groups_quar:
                # This is a group that was quarantined ('flagged') during
                # SelectTargets
                continue
            ...
            for side, contests in contests_sides.iteritems():
                ...
                if self.dirList == []:
                    self.template_width, self.template_height = Image.open(
                        exmpl_imP).size
                ...
                lst = [sorted(x, key=lambda x: (cols[x[2]], x[3]))
                       for x in lst]
    ...
    def start(self, project=None, projdir=None):
        ...
        self.imagebox = ImageManipulate(
            self, size=(self.view_x_size, self.view_y_size))
        ...
        self.remainingText = wx.StaticText(
            self, style=wx.TE_READONLY, size=(150, 30))
        ...
        self.templatebox = wx.Panel(self, size=(
            self.small_x_size, self.small_y_size))
        ...
        if os.path.exists(self.proj.patch_loc_dir):
            ...
            def find_next_regex(x=None):
                popup = wx.TextEntryDialog(None, "Enter the regex to match (filename|" + (
                    "|".join(map(str, attr_titles))) + ") where | is a newline", 'Title', '.*|.*|.*')

                if popup.ShowModal() == wx.ID_OK:
                    val = popup.GetValue()
                    val = val.split("|")
                    match = set([k for k, v in attr_data.items() if all(
                        re.match(x, y) for x, y in zip(val, [k] + list(v)))])
                    for time in range(2):
                        did = False
        ...
        def goto_num(x=None):
            popup = wx.TextEntryDialog(
                None, "Enter the style number to jump to", 'Title', '')
        ...
        def addmultibox(x):
            ...
            for bid in range(len(self.grouping_cached)):
                ...
                for cid in range(len(self.grouping_cached[bid]) - 1):
                    ...
                    m2 = self.mapping_inverse[
                        (bid, self.contest_order[bid][cid + 1])]
                    order.append((m1, m2))
                orders.append(order)
            # print "ORDS", orders
            ...
            boxes_in_new_group = [
                bid_cid for pair in extension for bid_cid in pair]
            cleared = []
            ...
            def putresults(get_result):
                ids_in_new_group = [x for x in get_result if any(
                    self.templatenum == y[0] for y in x)][0]
            ...
            def declareReady(x):
                ...
                for ct, cid_lst in enumerate(self.contest_order):
                    for cid in cid_lst:
                        ...
                        contests = ["_".join(["contest", str(ct), str(
                            cid), str(targ)]) for targ in range(numt)]
    ...
    def compute_equivs_2(self, run_verification=True):
        ...
        for ballot_count, ballot in enumerate(self.groupedtargets):
            ...
            for targetlist in ballot:
                ...
                targ_to_cont = get_target_to_contest(
                    flattencontests, [x[2:] for x in targetlist])
                corresponding_boxes = [flattencontests[x]
                ...
                # print 'w', w
                # print 'contest', targetlist[0][1], 'corresponds to',
                # contestbboxes[w[0]]
                for bbox in contestbboxes[w[0]]:
                    mapping[ballot_count, bbox] = (
                        ballot_count, targetlist[0][1])
        ...
        self.multiboxcontests = [[(c, self.contest_order[c][b])
                                  for c, b in y] for y in self.multiboxcontests_enter]

        self.equivs = [[mapping[bid, bboxes[0]]
                        for bid, bboxes in group] for group in gr]
    ...
    def setupBoxes(self):
        ...
        for i, ballot in enumerate(self.groupedtargets):
            ...
            remaining = max(ballotw - (2 * leftmargin),
                            0.10 * self.template_height)
            # The size of one box
    ...
    def nexttemplate(self, ct):
        ...
        self.imgo_resize = scipy.misc.imresize(
            self.imgo, (self.small_y_size, self.small_x_size))
        self.imgo_resizefactor_y = float(
            self.imgo.shape[0]) / self.imgo_resize.shape[0]
        self.imgo_resizefactor_x = float(
            self.imgo.shape[1]) / self.imgo_resize.shape[1]

        for cid in self.contest_order[self.templatenum]:
            # Fill in the current contest keys to use to index in the
            # hashtables.
    ...
    def updateTemplate(self):
        ...
        for l, u, r, d in self.boxes[self.templatenum]:
            ...
            if c == self.count:
                ...
                img[u:d, l:r] = img[u:d, l:r] + np.array([0, 200, 0]) * .5
            else:
                img[u:d, l:r] = img[u:d, l:r] * .5
                # dr.rectangle(box, fill=(200,0,0))
                img[u:d, l:r] = img[u:d, l:r] + np.array([200, 0, 0]) * .5
        ...
        if len(bothcontests) > 1:
            nextbox = [x for x in bothcontests if x !=
                       self.currentcontests[self.count]][0]
            l, u, r, d = fix(self.boxes[nextbox[0]][
                             self.contest_order[nextbox[0]].index(nextbox[1])])
    ...
    def restoreText(self):
        ...
        self.text_upto.SetValue(
            int(self.voteupto[self.currentcontests[self.count]]))
    ...
    def saveText(self, removeit=True):
        ...
        try:
            ...
            return
        v = [self.text_title.GetValue()] + [x.GetValue()
                                            for x in self.text_targets]
        ...
        for contest in self.currentcontests:
            if contest in self.text and sorted(self.text[contest]) == sorted(v) and contest != self.currentcontests[self.count]:
                dlg = wx.MessageDialog(
                    self, message="Did not save; contest is a duplicate of previous contest on this ballot.", style=wx.OK)
                dlg.ShowModal()
                return
        ...
        self.voteupto[self.currentcontests[
            self.count]] = self.text_upto.GetValue()
        ...
        text = [self.text[x][1:] if self.text[x] != [] else [
            ''] * len(self.groupedtargets[x[0]][self.contest_order[x[0]].index(x[1])]) for x in cur]
        text = sum(text, [])
        # print 'this', text
        ...
        if any(x in y for x in cur for y in self.equivs):
            ...
            for continuation in eqclass:
                ...
                index = 0
                each_in_dict = [
                    x for x in continuation if x in self.reorder[set_repr]][0]
                ...
                for each in sorted(continuation, key=lambda x: self.contest_order[x[0]].index(x[1])):
                    size = len(self.groupedtargets[each[0]][
                               self.contest_order[each[0]].index(each[1])])
                    # print 'assign', size, 'of them starting from', index,
                    # ':', [title]+newtext[index:index+size]
                    self.text[each] = [title] + newtext[index:index + size]
                    self.voteupto[each] = self.voteupto[
                        self.currentcontests[self.count]]
    ...
    def changeCompleted(self, addone=0):
        ...
        self.curBlankBallotNum.SetLabel(
            "On Style {0} of {1}".format(self.templatenum + 1, num_blanks))
    ...
    def addText(self):
        ...
        self.contesttitle = wx.StaticText(
            self.textarea, label="Contest Title", pos=(0, 0))
        ...
        self.text_title = wx.ComboBox(self.textarea, -1,
                                      choices=list(
                                          Set([x[0] for x in self.text.values() if x and len(x) - 1 == number_targets])),
                                      style=wx.CB_DROPDOWN, pos=(0, 25))
        self.text_title.Bind(wx.EVT_COMBOBOX, lambda x: changeOptions(
            x, override=True, fromcombo=True))
        ...
        def attempt_prefill(x):
            ...
            if self.text[self.currentcontests[self.count]] != []:
                return

            if any([self.currentcontests[self.count] in x for x in self.multiboxcontests]):
                return
            ...
            for template in range(len(self.dirList)):
                ...
                if prev is None:
                    ...
                    if len(maybe) - 1 == len(self.text_targets):
                        possible.append(
                            (tuple(maybe), self.voteupto[template, order[0]]))
                    continue
                for i in range(1, len(order)):
                    ...
                    if self.text[template, order[i - 1]] == prev:
                        ...
                        # print 'yes', len(maybe)
                        if any([(template, order[i]) in x for x in self.multiboxcontests]):
                            continue
                        if len(maybe) - 1 == len(self.text_targets):
                            possible.append(
                                (tuple(maybe), self.voteupto[template, order[i]]))
            if len(possible) != 0:
                (best, uptoval), ct = max(
                    Counter(map(tuple, possible)).items(), key=lambda x: x[1])
        ...
        if number_targets == 0:
            self.text_upto = wx.lib.intctrl.IntCtrl(
                self.textarea, pos=(0, -10000))
            return
...
rotation doesn't currently work for multibox contests.")
                        dlg.ShowModal()
                        return
                    pos = self.curtext_matched.index(
                        self.text_targets[j].GetValue())
                    wi = len(
                        [name for name in self.curtext_matched if is_writein(name)])
                    if wi == 0:
                        ...
                        # candidate name ordering on the "other" column as
                        # well.
                        neworder = self.curtext_matched[
                            pos:] + self.curtext_matched[:pos]
                    else:
                        # Ensure that the writein candidates always stay at the
                        # end
                        neworder = self.curtext_matched[
                            pos:-wi] + self.curtext_matched[:pos]
        ...
        t = wx.StaticText(self.textarea, label="Vote for up to",
                          pos=(0, 25 + 95 + (1 + i) * 25))
        sz.Add(t)
    ...
    def nextunfilled(self, x=None):
        ...
        aftertext = [x for x in aftertext if x >
                     (self.templatenum, self.count)]
        # Pick the first.
        if len(aftertext) == 0:
            return
