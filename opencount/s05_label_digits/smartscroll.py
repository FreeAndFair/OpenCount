...
class SmartScrolledGridPanel(ScrolledPanel):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        for evt in (wx.EVT_SCROLLWIN_TOP,
            ...
            self.Bind(evt, lambda evt: self.onScroll(
                evt, pos=self.CalcUnscrolledPosition((0, 0)), isScrollPos=False))
        for evt in (wx.EVT_SCROLLWIN_THUMBTRACK,
                    wx.EVT_SCROLLWIN_THUMBRELEASE):
            self.Bind(evt, lambda evt: self.onScroll(
                evt, pos=evt.GetPosition(), isScrollPos=True))
    ...
    def restore_state(self, state):
        ...
        dummy_spacer = (
            self.pagesize[1] * self.cellsize[0], self.pagesize[0] * self.cellsize[1])
        ...
        # of panels.
        num_activecells_max = (2 * self.lookahead + 1) * \
            self.pagesize[0] * self.pagesize[1]
        for _ in xrange(num_activecells_max):
            self.panels_pool.append(ImagePanel(
                self, color="red", size=self.cellsize))

        x_amt, y_amt = self.cellsize[0] / 10, self.cellsize[1] / 10
        self.SetupScrolling(scroll_x=True, scroll_y=True,
                            rate_x=x_amt, rate_y=y_amt)
    ...
    def activate_page(self, pageidx):
        ...
        for cellid in cellids:
            ...
            if type(imgpath) == int:
                ...
                panel.configure_me(cellid=cellid, color=clr, text="Page={0}".format(
                    pageidx), size=self.cellsize)
            else:
    ...
    def mark_cell_manually(self, cellid, labels):
        ...
        self.cellid2boxes[cellid] = [
            [None, None, None, None, label] for label in labels]
    ...
    def check_pages(self, pos):
        ...
        pageidxs = set(xrange(max(0, pageidx_cur - self.lookahead),
                              min(self.num_pages_total, pageidx_cur + self.lookahead + 1)))
    ...
    def rescale(self, newscale):
        ...
        """
        w_new, h_new = int(round(self.cellsize_orig[
                           0] * newscale)), int(round(self.cellsize_orig[1] * newscale))
        self.cellsize = (w_new, h_new)
        self.scale = newscale
        pagesize = (self.pagesize[1] * self.cellsize[0],
                    self.pagesize[0] * self.cellsize[1])

        # self.sizer contains either BoxSizers (active pages) or dummyspacers
        # (inactive pages)
        for idx_page, item_page in enumerate([s for s in self.sizer.GetChildren()]):
            if item_page.IsSpacer():
                # This is a dummy inactive page -- replace with a resized
    ...
    def onScroll(self, evt, pos=None, isScrollPos=False):
        ...
        if not isScrollPos:
            ...
            if orient == wx.HORIZONTAL:
                ...
                # OK to 'ignore' X val, as we never use it.
                pos = (0, pos * y_rate)
        ...
        max_pageidx = min(self.num_pages_total,
                          cur_pageidx + self.lookahead + C)
        pages_prefetch = tuple(xrange(min_pageidx, cur_pageidx - self.lookahead)) + \
            tuple(xrange(cur_pageidx + 1 + self.lookahead, max_pageidx))
...
class ImagePanel(wx.Panel):
    ...
    def __init__(self, parent, cellid=None, color="red", npimg=None, text=None, size=(100, 50), *args, **kwargs):
        ...
        m_labelmanual = self.popup_menu.Append(-1,
                                               "Manually Label.../Undo Manual Label")
        m_printpath = self.popup_menu.Append(-1, "(Dev) Print This Patch Path")

        self.Bind(wx.EVT_MENU, lambda evt: self.view_image_context(),
                  m_viewcontext)
        ...
        self.configure_me(cellid=self.cellid, color=self.color,
                          npimg=self.npimg, text=self.text, size=self.size)
    ...
    def configure_me(self, cellid=None, color="red", npimg=None, text=None, size=(100, 50)):
        ...
        if self.npimg != None:
            ...
            self.bitmap = wx.BitmapFromImage(wx.ImageFromBuffer(
                npimg_rsz_rgb.shape[1], npimg_rsz_rgb.shape[0], npimg_rsz_rgb))
        else:
            self.bitmap = None
            self.scale = self.GetParent().cellsize_orig[
                0] / float(self.size[0])
        self.Refresh()
        self.Layout()
...
choose the 'Undo Manual Label' option.")
            ...
            if self.get_boxes():
                ...
                box_sel = get_closest_box(
                    self.get_boxes(), (ximg, yimg), mode='within')
                if box_sel:
                    ...
                    self.boxes_sel, self.is_selecting = [], False
                    self.box_regionsel, self.is_regionselecting = [
                        x, y, x + 1, y + 1], True
            else:
                self.boxes_sel, self.is_selecting = [], False
                self.box_regionsel, self.is_regionselecting = [
    ...
    def onLeftUp(self, evt):
        ...
        if self.is_creating:
            ...
            box_region_canon = self.c2img(
                *canonicalize_box(self.box_regionsel))
    ...
    def onMotion(self, evt):
        ...
        if self.is_creating and evt.LeftIsDown():
            ...
            for box in self.boxes_sel:
                box[:4] = box[0] + xdelt, box[1] + \
                    ydelt, box[2] + xdelt, box[3] + ydelt
...
each letter/digit (i.e. the smallest unit) with a comma. For instance:",
        ...
        self.txtctrl = wx.TextCtrl(self, size=(
...
def main():
    ...
    app = wx.App(False)
    f = MainFrame(None, imgpaths, args.rows_page, args.lookahead,
                  args.num_cols, args.cellsize, args.no_prefetch)
    f.Show()
    app.MainLoop()
