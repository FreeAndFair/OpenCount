...
def trimmedMean(arr):
    ...
    if (math.isnan(mean)):
        return 0.0
    weights = [(1 if (x >= bottomCutoff and x <= topCutoff) else 0)
               for x in arr]
...
def crop(image, hCrop, vCrop):
    ...
    hCrop = int(hCrop)
    vCrop = int(vCrop)
    cv.SetImageROI(image, (hCrop, vCrop, image.width -
                           2 * hCrop, image.height - 2 * vCrop))
...
def makeThumbnail(imageMat, resizeFactor=1):
    thumbnail = cv.CreateMat(
        int(imageMat.rows / resizeFactor), int(imageMat.cols / resizeFactor), cv.CV_8UC1)
...
def makeBinary(imageMat):
    ...
    cv.Threshold(imageMat, binThumb, BIN_THRESHOLD,
                 BIN_COLOR, cv.CV_THRESH_BINARY_INV)
    return binThumb
...
def houghTransform(binaryImg, rho, theta, maxAngle, guess, method=METHOD_MEAN, graphImg=None):
    ...
    lines = lineDetect.findLines(binaryArray, rho, math.radians(
        theta), minAccumulator, math.radians(maxAngle), math.radians(guess))
...
def detectRotation(path, resizeFactor=1, maxAngle=ROT_WINDOW, outputDir="."):
    ...
    angle1 = houghTransform(binThumb, 1, 0.1, maxAngle,
                            0.0, METHOD_TMEAN, graphImg)

    if GRAPH:
        cv.SaveImage(os.path.join(
            outputDir, 'binary_{0}{1}'.format(filename, ext)), binThumb)
        cv.SaveImage(os.path.join(
            outputDir, 'lines_{0}{1}'.format(filename, ext)), graphImg)
    ...
    angle2 = houghTransform(binThumb, 1, 0.01, 0.1,
                            angle1, METHOD_MEDIAN, graphImg)

    if GRAPH:
        cv.SaveImage(os.path.join(
            outputDir, 'lines_pass2_{0}{1}'.format(filename, ext)), graphImg)
...
by padding/cropping the output image appropriately.")
    ...
    straighten_image(input, output, resize=resize,
                     maxAngle=maxAngle, imgsize=imgsize)
