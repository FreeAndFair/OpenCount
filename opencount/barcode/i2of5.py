...
def decode_i2of5(img, n, topbot_pairs, orient=VERTICAL, debug=False,
    ...
    for curcol in xrange(cols):
        ...
        decodings.append(decode_barcode(
            img, n, bc_loc[:], xoff=x1, imgP=imgP, debug=debug))
...
def decode_barcode(img, n, bc_loc, xoff=0, debug=False, imgP=None):
    ...
    # Number of stripes, including (7) for top/bot guards
    UPPER_LIMIT = n * 5 + 7
    ...
    if (len(whts) + len(blks) < UPPER_LIMIT) and curlen > 1:
        # Add the upper-most black stripe, which didn't get added inside the
        # for-loop
        blks_ys.append(h_imgpost - (idx + start_idx - 1) - 1)
        blks.append(curlen)
...
def compute_stripe_bbs(whts_ys, blks_ys, bars_wht, bars_blk, width, xoff,
    ...
    whiteNarrows, whiteWides = helper(
        whts_ys, bars_wht, width, wht_narrow, wht_wide, 'White')
    blackNarrows, blackWides = helper(
        blks_ys, bars_blk, width, blk_narrow, blk_wide, 'Black')
...
def find_barcode_loc_tm(I, bc_height, TOP_GUARD, BOT_GUARD,
    ...
    top_mats = get_tempmatches(
        TOP_GUARD, I, T=0.86, do_smooth=True, xwin=3, ywin=3, atleastone=True)
    cv.SetImageROI(I, _ROI)
    _ROI = cv.GetImageROI(I)
    cv.SetImageROI(I, (_ROI[0], _ROI[1] + h / 2, w, h / 2))
    bot_mats = get_tempmatches(
        BOT_GUARD, I, T=0.86, do_smooth=True, xwin=3, ywin=3, atleastone=True)
    cv.SetImageROI(I, _ROI)
...
def stripes_to_bars(lens, narrow, wide, imgP=None, T=0.04):
    ...
    for i, stripe_len in enumerate(lens):
        # -1.0 is perfect narrow, 1.0 is perfect wide
        s = get_score(stripe_len, narrow, r)
        if abs(s) >= T:
