...
class GridShow(wx.ScrolledWindow):
    ...
    def lightBox(self, i, evt=None):
        ...
        imgpaths = sorted(self.bal2imgs[balid],
                          key=lambda imP: self.img2page[imP])
        ballotpath = imgpaths[side]

        if not os.path.exists(ballotpath):
            dlg = wx.MessageDialog(
                self, message="Oh no. We couldn't open this ballot for some reason ...", style=wx.ID_OK)
            dlg.ShowModal()
            return
        ...
        for (ind, locs, tid) in other_stuff:
            ...
            # targetID name -> index in the list to find if it is 'wrong' or
            # not.
        ...
        templatepath = self.get_meta(
            self.img2bal[ballotpath], page)['template']
        wx.StaticText(pan, label="Ballot image:\n" + lines(ballotpath) + "\n\nTemplate image:\n" + lines(templatepath) + "\n\n" + ifflipped,
                      pos=(before.size[0], before.size[1] / 3))

        b = wx.Button(pan, label="Back", pos=(
            before.size[0], 2 * before.size[1] / 3))
        b.Bind(wx.EVT_BUTTON, remove)

        q = wx.Button(pan, label="Quarantine", pos=(
            before.size[0], 2 * before.size[1] / 3 + 50))
    ...
    def drawWrongMark(self, i):
        ...
        target_np = np.fromstring(jpg.tostring(), dtype='uint8').reshape(
            jpg.size[1], jpg.size[0], 3)
        x1, y1 = self.targetw * (i - imgIdx), 0
        x2, y2 = self.targetw * (i - imgIdx) + \
            self.targetw - 1, self.targeth - 1
    ...
    def drawWrongMarks(self, idxs, C=0.7):
        ...
        for imgIdx, idxs in imgIdx2idxs.iteritems():
            ...
            row_np = np.fromstring(pilimg.tostring(), dtype='uint8').reshape(
                pilimg.size[1], pilimg.size[0], 3)
            for i in idxs:
                x1, y1 = self.targetw * (i - imgIdx), 0
                x2, y2 = self.targetw * (i - imgIdx) + \
                    self.targetw - 1, self.targeth - 1
    ...
    def drawThreshold(self):
        if self.threshold not in self.index_to_visible:
            return
        cthreshold = self.index_to_visible[self.threshold]
        imgIdx = cthreshold - (cthreshold % self.numcols)
        if not (imgIdx in self.jpgs and imgIdx in self.images):
            return
    ...
    def setLine(self, which, evt=None):
        if evt is None:
            ...
            imgIdx = self.index_to_visible[
                which] - (self.index_to_visible[which] % self.numcols)
        else:
            imgIdx = which
            which = which + \
        ...
        if dlg.GetValue()[0] == 0:
            ...
            for each in range(self.numberOfTargets):
                ...
                if (each < self.threshold) ^ (each in self.wrong):
                    # print 'there a target', each, bid, ballot_to_group[bid],
                    # tid, contest
                    over_count[bid, side, contest].append(each)
            for (bid, side, contest), targs in over_count.items():
                upto = 1
                if has_contest_data:
                    global_cid = contest_id[self.bal2imgs[group_examples[
                        ballot_to_group[bid]][0]][side], str(contest)][0]
            ...
            dims = map(int, open(self.proj.extractedfile +
                                 ".size").read().strip()[1:-1].split(","))
            size = dims[0] * dims[1]
            ...
            sigmas = []
            for i in range(size):
                mu = np.sum(counts[i, :] * np.arange(256)) / \
                    np.sum(counts[i, :])
                sigma = (
                    np.sum(counts[i, :] * (np.arange(256) - mu)**2) / np.sum(counts[i, :]))**.5
                sigmas.append(sigma)
            print sigmas
            ...
            hist = np.bincount(
                (((targets - np.min(targets)) / (np.max(targets) - np.min(targets))) * 256).astype(np.int32))
            print hist
    ...
    def findBoundry(self, usedata=None):
        ...
        def matchnormal(data):
            ...
            if sum(data) == 0:
                return 0
            if sum([1 if x != 0 else 0 for x in data]) == 1:
                return 0
    ...
    def setup(self):
        ...
        self.classified_file = [((int(bid), int(side), int(tid)), int(
            value)) for bid, side, tid, value in self.classified_file]
        self.classified_lookup = dict(
            [(l, i) for i, (l, _) in enumerate(self.classified_file)])
    ...
    def onScroll(self, pos=None, evtpos=None):
        ...
        self.lastpos = pos
        low = max(0, pos - GAP)
        high = min(pos + self.numcols * self.numrows +
                   GAP, self.numberOfVisibleTargets)
    ...
    def dosave(self):
        ...
        2.) quarantined_manual.csv
            ...
            the SetThreshold panel. Each line is of the form:
                ...
                ...
        """
        if not self.somethingHasChanged:
            return
        self.somethingHasChanged = False
...
class ThresholdPanel(ffwx.Panel):
    ...
    def start(self, project=None, projdir=None):
        ...
        button3.Bind(wx.EVT_BUTTON, lambda x: tabOne.onScroll(
            tabOne.lastpos - tabOne.numcols * (tabOne.numrows - 5)))
        button4 = wx.Button(self, label="Scroll Down")
        button4.Bind(wx.EVT_BUTTON, lambda x: tabOne.onScroll(
            tabOne.lastpos + tabOne.numcols * (tabOne.numrows - 5)))
