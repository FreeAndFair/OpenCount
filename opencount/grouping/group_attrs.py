...
def cluster_imgpatchesV2(imgpaths, bb_map, init_clusters=None, THRESHOLD=0.95):
    ...
    while unlabeled_imgpaths:
        ...
        # Manually add in I
        matches.append((curimgpath, -1.0, 1.0, None,
                        bb[0], bb[1], bb[2], bb[3], 1.0))
        if matches:
            ...
            for (filename, sc1, sc2, Ireg, y1, y2, x1, x2, rszFac) in matches:
                y1, y2, x1, x2 = map(lambda c: int(
                    round(c / rszFac)), (y1, y2, x1, x2))
                if filename not in bestmatches:
                    bestmatches[filename] = (
                        filename, sc1, sc2, y1, y2, x1, x2, rszFac)
                else:
...
def group_attributes_V2(project, job_id=None, THRESHOLD=0.95):
    ...
    # maps {str attrtype: {str c_imgpath: [(imgpath_i, bb_i, score_i), ...]}}
    attr_clusters = {}
    ...
    for attr in ballot_attrs:
        ...
        clusters = cluster_imgpatchesV2(
            blank_imgpaths, bb_map, THRESHOLD=THRESHOLD)
...
def cluster_bkgd(mapping, bb_map=None, D=5, debug_SKIP=False):
    ...
    for label, imgpaths in mapping.iteritems():
        ...
        # [[(str imgpath_i0, float feat_i0), ...], [(str imgpath_i1, float feat_i1), ...], ...]
        clusters = []
